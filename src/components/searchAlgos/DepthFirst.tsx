import { depthFirstSearch } from "@/algorithms/searches/depthFirst";
...
const DepthFirst = () => {
  ...
  const [dsResult, setDSResult] = useState<DSResult[]>(defaultDSResult);

  useEffect(() => {
    const result: DSResult[] = depthFirstSearch(tree, 6);

    setDSResult(result);
  }, []);

  useEffect(() => {
    if (counter < dsResult?.length - 1) {
  ...
  return (
    <div className="flex flex-col items-center justify-evenly w-full h-full">
      <div className="flex flex-col items-center text-lg">
        {dsResult[counter].value === dataTree[0] ? (
        ...
        <div className="flex flex-row gap-10">
          <div>
            {dsResult[counter].value === dataTree[1] ? (
              <div className="font-bold border-solid border-red-500 border-2">
                {dataTree[1]}
              </div>
            ) : (
          ...
          <div>
            {dsResult[counter].value === dataTree[4] ? (
              <div className="font-bold border-solid border-red-500 border-2">
                {dataTree[4]}
              </div>
            ) : (
        ...
        <div className="flex flex-row gap-5">
          <div>
            {dsResult[counter].value === dataTree[2] ? (
              <div className="font-bold border-solid border-red-500 border-2">
                {dataTree[2]}
              </div>
            ) : (
          ...
          <div>
            {dsResult[counter].value === dataTree[3] ? (
              <div className="font-bold border-solid border-red-500 border-2">
                {dataTree[3]}
              </div>
            ) : (
          ...
          <div>
            {dsResult[counter].value === dataTree[5] ? (
          ...
          <div>
            {dsResult[counter].value === dataTree[6] ? (
              <div className="font-bold border-solid border-red-500 border-2">
                {dataTree[6]}
              </div>
            ) : (
